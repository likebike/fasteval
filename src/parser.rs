...
pub fn parse(slab:&mut ParseSlab, s:&str) -> Result<ExpressionI, KErr> {
    ...
    read_expression(slab, &mut bs, 0, true)
}
...
fn read_expression(slab:&mut ParseSlab, bs:&mut &[u8], depth:usize, expect_eof:bool) -> Result<ExpressionI, KErr> {
    ...
    let first = read_value(slab,bs,depth).map_err(|e| e.pre("read_value"))?;
    let mut pairs = Vec::<ExprPair>::with_capacity(8);
    loop {
        match read_binaryop(bs).map_err(|e| e.pre("read_binaryop"))? {
            Pass => break,
            Bite(bop) => {
                let val = read_value(slab,bs,depth).map_err(|e| e.pre("read_value"))?;
...
fn read_value(slab:&mut ParseSlab, bs:&mut &[u8], depth:usize) -> Result<Value, KErr> {
    ...
    match read_unaryop(slab,bs,depth)? {
        Pass => {}
        Bite(u) => return Ok(EUnaryOp(u)),
    }
    match read_callable(slab,bs,depth)? {
        Pass => {}
...
fn read_const(bs:&mut &[u8]) -> Result<Tok<Constant>, KErr> {
    spaces(bs);
    ...
    if suffix_ok {
        match peek_n(bs,toklen) {
            ...
            Some(b) => {
                ...
                if exp!=0 {
                    let expstring = exp.to_string();
                    char_buf = String::with_capacity(tok.len() + 1 + expstring.len());
                    char_buf.push_str(tok);
                    char_buf.push('e');
                    char_buf.push_str(&expstring);
                    tok = &char_buf;
...
fn read_unaryop(slab:&mut ParseSlab, bs:&mut &[u8], depth:usize) -> Result<Tok<UnaryOp>, KErr> {
    ...
    match peek(bs) {
        ...
        Some(b) => match b {
            b'+' => {
                skip(bs)?;
                let v = read_value(slab,bs,depth+1)?;
            ...
            b'(' => {
                skip(bs)?;
                let xi = read_expression(slab,bs,depth+1,false)?;
                spaces(bs);
...
fn read_callable(slab:&mut ParseSlab, bs:&mut &[u8], depth:usize) -> Result<Tok<Callable>, KErr> {
            ...
            match read_open_parenthesis(bs)? {
                ...
                Bite(_) => {
                    ...
                    match varname.0.as_ref() {
                        "print" => Ok(Bite(EPrintFunc(read_printfunc(slab,bs,depth)?))),
                        "eval" => Ok(Bite(EEvalFunc(read_evalfunc(slab,bs,depth)?))),
                        _ => Ok(Bite(EStdFunc(read_func(varname,slab,bs,depth)?))),
                    }
                }
            }
        }
...
fn read_varname(bs:&mut &[u8]) -> Result<Tok<VarName>, KErr> {
    spaces(bs);

    let mut toklen = 0;
    while is_varname_byte_opt(peek_n(bs,toklen),toklen) { toklen+=1; }
...
fn read_open_parenthesis(bs:&mut &[u8]) -> Result<Tok<()>, KErr> {
    spaces(bs);
...
fn read_func(fname:VarName, slab:&mut ParseSlab, bs:&mut &[u8], depth:usize) -> Result<StdFunc, KErr> {
    ...
    loop {
        ...
        args.push(read_expression(slab,bs,depth+1,false).map_err(|e| e.pre("read_expression"))?);
    }
...
fn read_printfunc(slab:&mut ParseSlab, bs:&mut &[u8], depth:usize) -> Result<PrintFunc, KErr> {
    ...
    loop {
        ...
        args.push(read_expressionorstring(slab,bs,depth+1)?);
    }

    Ok(PrintFunc(args))
}

fn read_evalfunc(slab:&mut ParseSlab, bs:&mut &[u8], depth:usize) -> Result<EvalFunc, KErr> {
    let eval_expr = read_expression(slab,bs,depth+1,false)?;
    ...
    loop {
        ...
        let expr = read_expression(slab,bs,depth+1,false)?;
...
fn read_expressionorstring(slab:&mut ParseSlab, bs:&mut &[u8], depth:usize) -> Result<ExpressionOrString, KErr> {
    match read_string(bs)? {
        Pass => {}
        Bite(s) => return Ok(EStr(s)),
    }
    Ok(EExpr(read_expression(slab,bs,depth+1,false)?))
}
...
mod internal_tests {
    ...
    fn priv_tests() {
        ...
        
        {
            let bsarr = b"12.34";
            let bs = &mut &bsarr[..];
            assert_eq!(read_value(&mut slab.ps, bs, 0), Ok(EConstant(Constant(12.34))));
        }
    }
}
